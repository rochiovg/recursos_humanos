...
if objFSO.FileExists("\\srv-fileserver\imagenes\CCRRHH\pdf_general\local_"&id&".pdf" ) then 
...
function validaArchivo (){
	...
	if(window.parent.document.frmMiForm.txt_textM.value == '' )
		 {
			 alert("Selecciona el texto que se va a mostrar como link");
			 window.parent.document.frmMiForm.txt_textM.focus();
			 return false;
	} else {
...
function verificar() {
	...
	if (a11==1 && a1==1 ) {
		...
		var texto=window.parent.document.frmMiForm.txt_textM.value;
		var id=window.parent.document.frmMiForm.id.value;
		...
		ajax=http_request();
		var valores;	
		valores= "texto="+escape(texto)+"&id="+escape(id)+"&lsTipo=122";  // parametros a enviar al servidor	
    ...
    ajax.onreadystatechange = function() {
         if (ajax.readyState==1) {
                 document.getElementById("reg2").innerHTML ="<span class=etiqueta>Cargando......</span>";
         }
         else if (ajax.readyState==4){
            if(ajax.status==200){
					alert ('Se cargo correctamente el archivo');	
...
<body onLoad="verificar()" >
	<form ENCTYPE="multipart/form-data" name="frmMisArchivos" id="frmMisArchivos" METHOD="POST" ACTION="recibir_archivoPDF.asp">
