...
if id_proyecto = 139 then
	...
	consulta1 = "EXEC dbo.pa_ficha_capacita_contrato_ednom '" & cargo & "', '" & varccdd & "', '" & varccpp & "', '" & varccdi & "', '"&id_proyecto&"', '"&lsConvCar&"'" 
	...
	consulta1 = "EXEC dbo.pa_ficha_capacita_contrato_ece'" & cargo & "', '" & varccdd & "', '" & varccpp & "', '" & varccdi & "', '"&id_proyecto&"', '"&lsConvCar&"'" 
	...
	If  Not RsP.BOF And Not RsP.EOF Then  %>
               ...
               <tr>
                  <td colspan="21" align="left" ><span class="titulo">NOTA DE EX&Aacute;MEN   &nbsp;&nbsp;&nbsp;<img src="images/excell1.gif" onclick="verReporte_cap_contrato(1)"  width="32" height="29" /></span></td>
                </tr>
                <tr>
...
<input type="text" class='e_texto' id='txtCapacitacion' name='txtCapacitacion' maxlength="5" value="<%=contadorPre%>" style="width:50" disabled="true"></div></td>
		      ...
		      <tr style="font-size:9px" >
				...
				<th bgcolor="#DAE8F3" class="dato3" >T.AULA</th>
                ...
                <th  bgcolor="#DAE8F3" class="dato3" >IMPUNTUALIDAD</th> 
                <th  bgcolor="#DAE8F3" class="dato3" >OBSERVACION</th> 
                <% else %>
                <th  bgcolor="#DAE8F3" class="dato3" >CRITERIO 1</th> 
                <th  bgcolor="#DAE8F3" class="dato3" >CRITERIO 2</th> 
                <th  bgcolor="#DAE8F3" class="dato3" >CRITERIO 3</th> 
                ...
                <%if id_rol=35  or id_rol=34 then 
                ...
                <th  bgcolor="#DAE8F3" class="dato3" >EVALUAR</th>
          		<%END IF%>
                 <%if id_proyecto <>139 then%>
                <th  bgcolor="#DAE8F3" class="dato3" >T.OPERATIVO</th>
                <%end if%>
          <%END IF%>
       ...
       END IF  %>
                ...
                <%if id_proyecto=103 then %> 
                 ...
                 <td align="center"  class='e_td_string1' id="td_eval_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input <%=varvisualiza%> readonly name="txteval<%=RsP.Fields("id_per")%>" class="etiqueta"  type="text" id="txteval<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("des_eval")%>"   size="5" maxlength="5" />
            </span></td>
                
                
                
                <% if  RsP.Fields("p1")="" or isnull(RsP.Fields("p1")) then 
				color=""
				else 
				color ="bgcolor=""#98CBA4"""
				end if
				 %> 
                <td align="center" <%=color%> class='e_td_string1' id="td_1_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input <%=varvisualiza%>  name="txtp1<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros_dec(event)" class="etiqueta"  type="text" id="txtp1<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p1")%>"  onblur="guardarCAPA('p1','<%=RsP.Fields("id_per")%>',this.value, '1', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="5" />
            </span></td>
            <% if  RsP.Fields("p2")="" or isnull(RsP.Fields("p2")) then 
				color1=""
				else 
				color1 ="bgcolor=""#98CBA4"""
				end if
				 %>
				 
            <td <%=color1%> class='e_td_string1' id="td_2_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input  <%=varvisualiza%> name="txtp2<%=RsP.Fields("id_per")%>" onKeyPress="return fs_numeros_dec(event)" class="etiqueta"  type="text" id="txtp2<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p2")%>"  onblur="guardarCAPA('p2','<%=RsP.Fields("id_per")%>',this.value, '2', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="5" />
            </span></td>
<% if  RsP.Fields("p3")="" or isnull(RsP.Fields("p3")) then 
				color2=""
				else 
				color2 ="bgcolor=""#98CBA4"""
				end if
				 %>
            <td <%=color2%> class='e_td_string1' id="td_3_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input <%=varvisualiza%> name="txtp3<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros_dec(event)" class="etiqueta"  type="text" id="txtp3<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p3")%>"  onblur="guardarCAPA('p3','<%=RsP.Fields("id_per")%>',this.value, '3', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="5" />
            </span></td>
            <% if  RsP.Fields("p4")="" or isnull(RsP.Fields("p4")) then 
				color3=""
				else 
				color3 ="bgcolor=""#98CBA4"""
				end if
				 %>
             <td <%=color3%> class='e_td_string1' id="td_4_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input  <%=varvisualiza%>  name="txtp4<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros_dec(event)" class="etiqueta"  type="text" id="txtp4<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p4")%>"  onblur="guardarCAPA('p4','<%=RsP.Fields("id_per")%>',this.value, '4', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="5" />
            </span></td>
            <% if  RsP.Fields("l1")="" or isnull(RsP.Fields("l1")) then 
				color4=""
				else 
				color4 ="bgcolor=""#98CBA4"""
				end if
				 %>
            <td <%=color4%> class='e_td_string1' id="td_5_<%=RsP.Fields("id_per")%>"><span class="etiqueta">
              <input <%=varvisualiza%> name="txtl1<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros(event)" class="etiqueta"  type="text" id="txtl1<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("l1")%>"  onblur="guardarCAPA('l1','<%=RsP.Fields("id_per")%>',this.value, '5', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="2" />
            </span></td>
            
            <% if  RsP.Fields("mod_cod_desc")="" or isnull(RsP.Fields("mod_cod_desc")) then 
				color5=""
				else 
				color5 ="bgcolor=""#98CBA4"""
				end if
				 %>
            
            <td <%=color5%> class='e_td_string1' id="td_6_<%=RsP.Fields("id_per")%>">
             <input  <%=varvisualiza%> name="txt4<%=RsP.Fields("id_per")%>" onKeyPress="return fs_SoloLetras(event)" class="etiqueta"  type="text" id="txtmod_cod_desc<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("mod_cod_desc")%>"  onblur="guardarCAPA('mod_cod_desc','<%=RsP.Fields("id_per")%>',this.value,'6',<%=RsP.Fields("id_convocatoriacargo")%>)" size="50" maxlength="300" />
            </td>
            <% else %>
            ...
            <%IF lsConvCar=1831 OR lsConvCar=1832 OR lsConvCar=1841 or plsCargo=2971 or lsConvCar=2697 or lsConvCar=2698 THEN %>
              <td width="207" class='e_td_string1'><span class="etiqueta">
              <input  <%=varvisualiza%>  name="txtp4<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros_dec(event)" class="etiqueta"  type="text" id="txtp4<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p4")%>"  onblur="guardarCAPA('p4','<%=RsP.Fields("id_per")%>',this.value, '4', <%=RsP.Fields("id_convocatoriacargo")%>)" size="5" maxlength="2" />
            </span></td>
            <td width="207" class='e_td_string1'><span class="etiqueta">
			...
			%>
                <td class='e_td_string1'><div id="txtprom<%=RsP.Fields("id_per")%>"><%=RsP.Fields("notacap")%></div></td>
...
<% if id_proyecto <>139 then%>
		<td  class='e_td_string1' align="center" id="t<%=RsP.Fields("ID_PER")%>1"><select <%=varvisualiza%>  style="font-size:10px"  name="<%=RsP.Fields("ID_PER")%>1" id="<%=RsP.Fields("ID_PER")%>" onfocus="valorinicial(this.value,this.id)" onchange="f_contador_pre2(this.id,'<%=RsP.Fields("nombre")%>','<%=RsP.Fields("ID_PER")%>',this.value,'<%=lsUsuario%>')">
                  <option value="9" <%if(RsP.Fields("Habil_prueba")="9") then response.write("selected")end if%>>Elija Opci&oacute;n...</option>
                  <option value="1" <%if(RsP.Fields("Habil_prueba")="1") then response.write("selected")end if%> >2P ECE</option>
                  <option value="2" <%if(RsP.Fields("Habil_prueba")="2") then response.write("selected")end if%>>2P MC</option>
                  <option value="3" <%if(RsP.Fields("Habil_prueba")="3") then response.write("selected")end if%>>4P SOLO L2</option>
				  <option value="4" <%if(RsP.Fields("Habil_prueba")="4") then response.write("selected")end if%>>2S ECE</option>
                  <option value="5" <%if(RsP.Fields("Habil_prueba")="5") then response.write("selected")end if%>>2S M</option>
