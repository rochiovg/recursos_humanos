...
'	Response.ContentType="text/html; charset=iso-8859-1"
	set wRsDptoapli = Micone.execute(" exec getcargoPresupuesto_nvo '"&idProyecto&"' ")
	...
	While not wRsDptoapli.EOF
		Response.Write "<option title="""&wRsDptoapli(5)&" ("&wRsDptoapli(2)&")" &" - Actividad: "&wRsDptoapli(4)& " - PEA LIBRE A CERTIFICAR: "&wRsDptoapli(6)& " - PERIODO: "&wRsDptoapli("DES_PERIODO")&""& " F.Fto:"&wRsDptoapli(7)&""" value=""" & wRsDptoapli("id_presupuestodet") & """>"&wRsDptoapli(1)&" ("&wRsDptoapli(2)&")" & "</option>"
		wRsDptoapli.MoveNext
	Wend
	Response.Write "</select>"
...
function narmadas(tiempo,tipoperiodo)
{
	...
	document.frmMiForm.txtnperiodo.value =ntiempo  ;
	document.frmMiForm.cmbperiodo1.value = tipoperiodo;
	
	cmbCargo = document.frmMiForm.cmbCargo.value ;
...
function armadas(valor)	
{
	ajax=http_request();
		var valores;	
	valores= "cmbCargo=" + escape(valor)+"&lsTipo=173";  // parametros a enviar al servidor	
    ...
    ajax.onreadystatechange = function() {
         if (ajax.readyState==1) {
            ...
            if(ajax.status==200){
				...
				$( "#users tbody" ).append(ajax.responseText ); 
				// document.getElementById("users tbody").innerHTML =ajax.responseText;
...
function topes(valor)
{
	...
	$( "#users tbody" ).empty();
		...
		ajax=http_request();
	var valores;	
	valores= "datos=" + escape(valor)+"&lsTipo=172";  // parametros a enviar al servidor	
    ...
    ajax.onreadystatechange = function() {
         if (ajax.readyState==1) {
            ...
            if(ajax.status==200){
					...
					tipoperiodo =   document.getElementById("cmbperiodo").value
					tiempo =   document.getElementById("txtnPermax").value
					
					  
					 narmadas(tiempo,tipoperiodo);
...
<table width="95%" id="users" border="1" align="center" cellpadding="2" cellspacing="2" bordercolor="#DAE8F3">
  <thead><tr>
 	    ...
 	    <tr>
 	      ...
 	      <td class='etiqueta'><input  disabled name="txtnperiodo" type="text" class="e_td_number" id="txtnperiodo" onKeyPress="return TextUtil.allowChars(this, event)" size="5" maxlength="5" validchars="1234567890,." />
 	        <select name="cmbperiodo1"  disabled class="e_combo" id="cmbperiodo1"  onChange="validaPeriodo(this.value);" >
...
<input name="txtarmadas" type="hidden" id="txtarmadas" value="">
         ...
         <tbody>
			 ...
			 <td colspan="4" align="center" >
                <input name="button" type="button" class="boton" id="button" value="Grabar" onClick="enviar_cargo_nvo()">
