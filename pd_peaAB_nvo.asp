...
id_Proyecto=Session("id_proyecto")
	...
	SQL1 = "execute uspGet_PEAAB_NVO '"&fec_reg &"','"&fec_exec &"', '"&id_Proyecto&"'"
...
var id_per=id.substring(1);
	...
	var valores;	
	valores= "lsidper=" +id_per+"&lsvalor="+escape(val)+"&lsTipo=177";  // parametros a enviar al servidor	
...
<%'----- Oficios de Contratación ----- %> 
    ...
    <th width="05%" bgcolor="#DAE8F3" class="dato3">Contrato</th>
    ...
    <td class='e_td_string1'><input name="<%="S"&RS3.Fields("id_contratos")%>" type="checkbox" id="<%="S"&RS3.Fields("id_contratos")%>" value="1" <% if RS3.Fields("a_contratar") ="1" then response.Write("checked=""checked""") end if %> onClick="bloqueo_contrato(this.id,this.value)" />
     <div style="display:none" id="<%="a"&RS3.Fields("id_contratos")%>"></div>
        </td>
