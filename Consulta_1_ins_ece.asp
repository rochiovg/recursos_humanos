...
varReporte=request("lsReporte") '7
	...
	txtDistSed=mid(txtDistSed,6,2)		
	'txtDistSed=mid(txtDistSed,1,6)		
	end if
		
	
	varDpto=mid(varNomDpto,1,3)		'15
	varProv=mid(varNomProv,4,2)		'01	
	varDist=mid(varNomDist,6,2)		'01
	'							
...
if varReporte="1" then
	...
	if vartiCargo = "9" then
		...
		SQL1 = "exec dbo.pa_reporte_procesos_nivel_ece '"&varCargo&"', '"&varDpto&"', '"&varProv&"', '"&varDist&"' , '"&varReporte&"', '"&vartiCargo&"', '"&idProyecto&"', '"&varlsconv&"', '"&valor&"', '"&lsnivel&"', '"&txtSedeop&"', '"&sede&"' , '"&txtProvSed&"', '"&txtDistSed&"', '"&txtfecha&"', '"&txtTrabI&"' "
	
	if idProyecto = 139 then
	SQL1 = "exec dbo.pa_reporte_procesos_nivel_ednom '"&varCargo&"', '"&varDpto&"', '"&varProv&"', '"&varDist&"' , '"&varReporte&"', '"&vartiCargo&"', '"&idProyecto&"', '"&varlsconv&"', '"&valor&"', '"&lsnivel&"', '"&txtSedeop&"', '"&sede&"' , '"&txtProvSed&"', '"&txtDistSed&"', '"&txtfecha&"', '"&txtTrabI&"' "
	end if
...
If Not RSCuad.BOF And Not RSCuad.EOF Then
	...
	%>
       ...
       <tr>
                  <td colspan="16" align="center"><span class="titulo">REPORTE DE <%= REPORTE %><img src="images/excell1.gif" onclick="enviar_consultaReporte_ece()"  width="32" height="29" /></span></td>
          </tr>
                ...
                <th  bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">Tiempo de Contrataci&oacute;n</th>
                    ...
                    <th  bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">S.Regional</th>
					<th bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">S.Provincial</th>
					<th   bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">S. Dist</th>                                
 ...
 <th  bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">Resultado</th>
		  ...
		  <%registro = 0					
			...
			response.Flush()
				If RSCuad.Fields("pobaja")="1" Then
					clase = "bgcolor=""#EBF0AA"""
				Else
					clase = ""
				End If	
				cont = registro+1 %>
...
%>
			     ...
			     <td class='e_td_string1'>&nbsp;<%=RSCuad.Fields("telefono")%></td>				
                  ...
                  <%end if %>
                  <% if varReporte=23 then%>
                  <td class='e_td_string1'><%=RSCuad.Fields("p_formacionAct")%></td>
