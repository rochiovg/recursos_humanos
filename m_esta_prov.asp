...
<%
  ...
  dpto = Request.Form("dpto")
	...
	consulta1 = "EXEC dbo.PA_COBERTURA_PROV '"&id_cc&"', '"&ccdd&"' "  
    ...
    <tr height="20px">
      <td align="center" valign="middle">
        <form action="reporte.asp" method="post" target="_blank" id="FormularioExportacion_">
          <img src="images/excell1.gif" style="cursor:pointer" title="EXPORTAR A EXCEL" onclick="javascript:exporta_esta_pro('<%=ccdd%>','<%=dpto%>')"/>
  ...
  <table width='80%'  align="left" id="Exportar_a_Excel" style="font-family:Arial; font-size:9px"  border="1" cellpadding="2" cellspacing="2" bordercolor="#DFDFDF">
  	<tr height="20px">
      <th colspan="23" bgcolor="#F2F3F4"><span class="aplicacion" style="height:30px">COBERTURA: DEPARTAMENTO <%=dpto%></span></th>
   	</tr>
   	<tr height="20px">
      <th colspan="23"  bgcolor="#F2F3F4"  ><span class="aplicacion"><a href="#" onclick="mapaDpto()" title="Ver Reporte Nacional">Retroceder</a></span></th>
   	</tr>
   <tr>
      <td width="30"  align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>Nro</strong></td>
      <td width="100" align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>PROVINCIA</strong></td>
   ...
   <tr>
      ...
      <% if RsP.Fields("prov")="TOTAL" or RsP.Fields("meta_contrato")="0" THEN %> 
      <td><%=RsP.Fields("prov")%> </td>
      <% else %>
      <td><a href="#" onclick="verDistritos('<%=id_cc%>','<%=RsP.Fields("ccdd")%>','<%=RsP.Fields("ccpp")%>','<%=dpto%>','<%=RsP.Fields("prov")%>')" title="Ver Distritos">
						<%=RsP.Fields("prov")%>
          </a>
	    </td>
      <% end if %>
