...
SQL4 = "exec dbo.getNombreProy '" & id_proyecto& "'  "	
	...
	SQL1 = "EXEC dbo.uspGet_PlanillaDetalle '"&lsperiodo&"','" & varccdd & "', '" & varccpp & "', '" & varccdi & "', '" & lsnivel& "', '" & id_proyecto& "'" 
	...
	SQL2 = "EXEC dbo.uspGet_PlanillaDetalleTotal '"&lsperiodo&"','" & varccdd & "', '" & varccpp & "', '" & varccdi & "', '" & lsnivel& "', '" & id_proyecto& "'" 
...
<form name="frmMiForm2" id="frmMiForm2" border="0" METHOD="POST" ACTION="pd_dtAprobarConformidad.asp">

<table border="1" cellpadding="0" style=" font-family:Arial; font-size:10px;" cellspacing="0" width="100%">

      <tr style="border:0;">
        <td height="61" colspan="13" align="center" style="font-size:15px;border:0;"><BR><strong>PROYECTO <%=RSCuad4.Fields("nombre")%>&nbsp;&nbsp;-&nbsp;&nbsp;APROBACION DE LA PRE CONFORMIDAD</strong>
		<strong><BR>
		PERIODO: <%=trim(RSCuad.Fields("periodo"))%></strong>
  ...
  <% else
   ...
   %>
      <tr>
        ...
        <td bordercolor="#DAE8F3" class='e_td_string1'>&nbsp;<%=trim(RSCuad.Fields("DNI"))%></td>
