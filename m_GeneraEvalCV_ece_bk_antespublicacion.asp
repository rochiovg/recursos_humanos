    ...
    <input type="text" class='e_texto' id='txtCapacitacion' name='txtCapacitacion' maxlength="5" value="<%=contadorPre%>" style="width:50" disabled="true"></div></td>
		      ...
		      <tr style="font-size:9px" >
                ...
                <th width="8%" rowspan="2"  bgcolor="#DAE8F3" class="dato3" >EVALUAR</th>
                 ...
                 <% if lsConvCar<>3576 and  lsConvCar<>3575 then %>
                ...
                <% end if%>
                  ...
                  <th width="8%" rowspan="2"  bgcolor="#DAE8F3" class="dato3" >EDAD MINIMA 20 A 55</th>
                <th width="8%" rowspan="2"  bgcolor="#DAE8F3" class="dato3" >PERFIL MINIMO</th>
                  <th width="8%" rowspan="2"  bgcolor="#DAE8F3" class="dato3"  >EXP. LAB. MIN</th>
       ...
       %>
			...
			colorT="bgColor=""#7EBA65"""
               ...
               <td  style="display:none" class='e_td_string1' id="tb"><div id="tr1<%=RsP.Fields("id_per")%>"><%=RsP.Fields("estado_cv")%></div></td>
                     ...
                     <% if lsConvCar=3576 or lsConvCar=3575 then %>
                <td class='e_td_string1'>
                
		          <input <%=varvisualiza%> type="checkbox" name="txt11<%=RsP.Fields("id_per")%>" class="etiqueta"   id="p_formacionAct<%=RsP.Fields("id_per")%>"  <%if RsP.Fields("p_formacionAct")="1" then response.write("checked") end if%>  onClick="guardarCVAplicador('p_formacionAct','<%=RsP.Fields("id_per")%>',this.value)" size="1"  /></td>
                  	
		        <td class='e_td_string1'><input <%=varvisualiza%> type="checkbox" name="p_formacionAct<%=RsP.Fields("id_per")%>" class="etiqueta"  id="p_expLaboral<%=RsP.Fields("id_per")%>" <%if RsP.Fields("p_expLaboral")="1" then response.write("checked") end if%>  onClick="guardarCVAplicador('p_expLaboral','<%=RsP.Fields("id_per")%>',this.value)" size="1"  /></td>
                
		        <td class='e_td_string1'><input <%=varvisualiza%>  type="checkbox" name="p_formacionAct<%=RsP.Fields("id_per")%>" class="etiqueta"   id="p_expAula<%=RsP.Fields("id_per")%>" <%if RsP.Fields("p_expAula")="1" then response.write("checked") end if%>  onClick="guardarCVAplicador('p_expAula','<%=RsP.Fields("id_per")%>',this.value)" size="1" /></td>
