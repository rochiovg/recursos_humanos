...
<%
   ...
   ambito = Request.Form("ambito")
	...
	consulta1 = "EXEC dbo.PA_COBERTURA_DPTO '"&id_cc&"', '"&ambito&"'  "  
...
<table width='89%' align="left" id="Exportar_a_Excel" style="font-family:Arial; font-size:10px"  border="1" cellpadding="2" cellspacing="2" bordercolor="#DFDFDF">
   <tr height="10px">
      <th colspan="18"  bgcolor="#F2F3F4"  ><span class="aplicacion" style="height:30px">COBERTURA A NIVEL NACIONAL</th>
   </tr>
   <tr>
      <th width="36" rowspan="2" bgcolor="#E4E4E4" style="font-size:11px;">Nro</th>
      <th rowspan="2" bgcolor="#E4E4E4" style="font-size:11px;">DEPARTAMENTO</th>
   ...
   <tr <%=col%> height="10px" >
      ...
      <td><%=RsP.Fields("dpto")%> </td>
      <% else %>
      <td><a href="#" onclick="verProvincias('<%=id_cc%>', '<%=RsP.Fields("ccdd")%>', '<%=RsP.Fields("dpto")%>')" title="Ver Provincias"><%=RsP.Fields("dpto")%></a>
	  </td>
      <% end if %>
