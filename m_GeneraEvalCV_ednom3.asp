...
cargo = Mid(plsCargo,2)
	...
	' CONTADOR
	SQLContCapac = "EXEC dbo.usp_count_EVALCV_cc '" & varccdd & "', '" & varccpp & "', '" & varccdi & "', '" & cargo & "', '"&id_proyecto&"', '"&lsConvCar&"'" 
...
<input type="text" class='e_texto' id='txtCapacitacion' name='txtCapacitacion' maxlength="5" value="<%=contadorPre%>" style="width:50" disabled="true"></div></td>
        ...
        <!-- <th width="11%" rowspan="2"  bordercolor="#DAE8F3"  bgcolor="#DAE8F3" class="dato3">LOCAL DE APLICACI&Oacute;N ASIGNADA</th>       -->
                ...
                <% if id_rol=0 or id_rol = 31 or id_rol = 1 or id_rol = 34  or id_rol = 38   then %>
              	...
              	<th width="8%" colspan="1"  bgcolor="#DAE8F3" class="dato3" >CUMPLIMIENTO DE REQUISITOS</th>
				...
				<th width="8%" bgcolor="#DAE8F3" class="dato3" >EXPERIENCIA EN APLICACIONES</th>
                 ...
                 <% if id_proyecto=139 then %>
                <th width="8%"  bgcolor="#DAE8F3" class="dato3" >FORMACI&oacute;N ACAD&Eacute;MICA</th>
                <%ELSE%>  
          ...
          <%end if%>
		      ...
		      <tr style="font-size:9px" >
		        ...
		        <th width="4%"  bgcolor="#DAE8F3" class="dato3" ></th>
...
<!--    			<td class='e_td_string1'>&nbsp;</td>-->
                ...
                <td class='e_td_string1'>
                  ...
                  <select <%=varvisualiza%>  style="font-size:10px"   name="txt11<%=RsP.Fields("id_per")%>"  id="p_formacionAct<%=RsP.Fields("id_per")%>"  onfocus="valorinicial(this.value,this.id)" onchange="guardarCV('p_formacionAct','<%=RsP.Fields("id_per")%>',this.value)">
                  <option value="0" <%if(RsP.Fields("p_formacionAct")="") then response.write("selected")end if%>>...</option>
                  <option value="1" <%if(RsP.Fields("p_formacionAct")="1") then response.write("selected")end if%> >SI</option>
                  <option value="2" <%if(RsP.Fields("p_formacionAct")="2") then response.write("selected")end if%>>NO</option>
                </select>
		        ...
		        <% if id_proyecto<>139 then %><td class='e_td_string1'><input <%=varvisualiza%> name="p_formacionAct<%=RsP.Fields("id_per")%>"validchars="0123456789" onkeypress="return TextUtil.allowChars(this, event)" class="etiqueta"  type="text" id="p_experiencia<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p_experiencia")%>"  onblur="guardarCV('p_experiencia','<%=RsP.Fields("id_per")%>',this.value)" size="5" maxlength="2" /></td><%end if%>
