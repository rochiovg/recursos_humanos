...
<%@ language="vbscript" %>
<!-- #include file="conexion_java.asp"  --> 
<%
...
id_Proyecto=1
	...
	if (alcance="") then	
		...
		'SQLQuery1=" exec dbo.usp_Meta_mapa '2973', '131', '6010' " 	
		SQLQuery1=" xxx_CARTO2015_GRAFICO_MAPA_NACIONAL "
	end if
	...
	Do While Not Rs1.Eof			
			...
			response.Write("<color>" & colorPoligono(arrayAvance5,arrayAvance1,estado) & "</color>")			
			response.Write("</dato>")
			Rs1.MoveNext			
	Loop	
	...
	function textoUbigeo(alcance,rs)
		if alcance="" then
			response.Write("<nom>" & rs("departamento") & "</nom>")
	...
	function colorPoligono(arrayAvance5,arrayAvance1,estado)
		...
		for i=0 to  ubound(arrayAvance1)
			sumMeta=sumMeta + CDbl(arrayAvance1(i))
		next

			
		for i=0 to  ubound(arrayAvance5)
			sumAvance=sumAvance + arrayAvance5(i)
			'sumestado=sumestado + estado(i)
		next
	
		
		if sumMeta>=0 and sumMeta<=24.99 then
			colorPoligono=4
		elseif sumMeta>=25 and sumMeta<=49.99 then 
			colorPoligono=3
		elseif sumMeta>=50 and sumMeta<=74.99 then 
			colorPoligono=2
	    else
	    	colorPoligono=1 ' Iniciado (blanco)
	    end if
