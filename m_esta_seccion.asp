...
<%
  ...
  dist = Request.Form("dist")
	...
	consulta1 = "EXEC dbo.PA_COBERTURA_SECCION '"&id_cc&"', '"&ccdd&"', '"&ccpp&"', '"&ccdi&"', '"& zona &"' "  
    ...
    <tr height="20px">
      <td align="center" valign="middle">
        <form action="reporte.asp" method="post" target="_blank" id="FormularioExportacion_">
          <img src="images/excell1.gif" style="cursor:pointer" title="EXPORTAR A EXCEL" onclick="javascript:exporta_esta_sec('<%=ccdd%>','<%=ccpp%>','<%=ccdi%>','<%=zona%>','<%=dpto+"*"+prov+"*"+dist+"*"+zona%>')"/>
...
<table width='80%' align="left" id="Exportar_a_Excel" style="font-family:Arial; font-size:9px"  border="1" cellpadding="2" cellspacing="2" bordercolor="#DFDFDF">
   <tr height="20px">
      <th colspan="23"  bgcolor="#F2F3F4"><span class="aplicacion">COBERTURA: DEPARTAMENTO <%=dpto%> - PROVINCIA <%=prov%> - DISTRITO <%=dist%> - ZONA <%=zona%></span>
      </th>
   </tr>
   <tr>
   <tr height="20px">
      <th colspan="23"  bgcolor="#F2F3F4"><span class="aplicacion">
        	<a href="#" onclick="verZonas ('<%=id_cc%>', '<%=ccdd%>', '<%=ccpp%>', '<%=ccdi%>', '<%=dpto%>', '<%=prov%>', '<%=dist%>')" title="Ver Reporte a nivel de zonas">Retroceder</a>
          </span>
      </th>
   </tr>
   <tr>
      <td width="30"  align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>Nro</strong></td>
      <td width="100" align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>SECCI&Oacute;N</strong></td>
   ...
   <tr <%=col%> height="16px">
      ...
      <% if RsP.Fields("dist")="TOTAL" or RsP.Fields("inscritos")="0" THEN %> 
      <td><%=RsP.Fields("dist")%> </td>
      <% else %>
      <td><a href="#" onclick="verInscritos('<%=id_cc%>','<%=RsP.Fields("ccdd")%>','<%=RsP.Fields("ccpp")%>','<%=RsP.Fields("ccdi")%>','<%=RsP.Fields("zona")%>','<%=RsP.Fields("seccion")%>' )" title="Ver Inscritos" ><%=RsP.Fields("seccion")%></a>
	  </td>
      <% end if %>
