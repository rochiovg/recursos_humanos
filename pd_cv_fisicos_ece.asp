...
IF id_proyecto= 139 or id_proyecto= 155 or id_proyecto= 156 then
...
sub generaRegion()
	...
	sql1 = " exec dbo.getCCDD_UBI '"&sedeReg&"' " 'ACTUALIZARLO CUANDO SE DEFINAN LAS SEDES OPERATIVAS
...
function generaConvocatoria(valor){     
	...
	url="m_numConvocatoriaECE16.asp";  
...
{    
 	...
 	if (txtFOLIO=="") 
	{
 	...
 	if (txtNROREG=="") 
	{
		alert('Debe de ingresar el NRO REG'); 
		return false; 
	} 
	...
	var valores="txtDpto1="+txtDpto1+"&txtProv1="+txtProv1+"&txtDist1="+txtDist1+"&txtConvocartoria="+txtConvocartoria+"&cmbCargo="+cmbCargo+"&txtDNI="+txtDNI+"&txtFOLIO="+txtFOLIO+"&txtNROREG="+txtNROREG;
	

	var peticion=$.ajax(
		{
		 url:'pd_InsertarCVFisicos_ECE.asp?nocache=' + Math.random(),
		...
		success:function (respuesta) {
			if ( respuesta == -1 ) {
				alert("No existe el DNI en la convocatoria");
			   ...
			   // Setea valor en vacio..
				...
				document.getElementById("txtNROREG").value = ""
...
//	var valores="id_proy="+id_proy+"&txtConvocartoria="+txtConvocartoria+"&txtDpto1="+txtDpto1+"&txtProv1="+txtProv1+"&txtDist1="+txtDist1;
	...
	var valores="txtDpto1="+txtDpto1+"&txtProv1="+txtProv1+"&txtDist1="+txtDist1+"&txtConvocartoria="+txtConvocartoria+"&cmbCargo="+cmbCargo+"&txtDNI="+txtDNI;
	

	var peticion=$.ajax(
		{
	     url:'pd_ListarCVSFisicos_ECE.asp?nocache=' + Math.random(),
		 timeout:7000,
		 type:'POST',
		 contentType: "application/x-www-form-urlencoded",
         data:valores,
...
function generaprov(){
 ...
 var lsubiprov=document.getElementById("lsubiprov").value;
	...
	url="m711_Provincia.asp";  //Cambiarlo a definir las sedes m_Provincia_ece
	ajax.open ('POST', url, true);
    ajax.onreadystatechange = function() {
         if (ajax.readyState==1) {
...
function verdis(){
 ...
 var lsUbiDist =document.getElementById("lsubidist").value;
	...
	url="m71_Distrito.asp";   //m_Distrito_ece
...
<div id="formulario">
  <div id="divNavegacion" align="center"><span class="subtitulo">Recepci&oacute;n de CV Fisicos</span></div>
    ...
    <tr>
                <td  align="right" bgcolor="#F7F8F9"><span class="criterio"><%IF id_proyecto= 139 or id_proyecto= 155 or id_proyecto= 156 THEN %> Departamento: <%else%>Sede Regional:<%end if%></span></td>
                <td bgcolor="#F7F8F9" class='criterio'><%generaRegion()%></td>
              </tr>
              <tr>
                <td  align="right" bgcolor="#F7F8F9"id="nomreg2"><span class="criterio"><%IF id_proyecto= 139 or id_proyecto= 155 or id_proyecto= 156THEN %>Provincia: <%else%>Sede Provincial:<%end if%></span></td>
              ...
              <tr>
                <td  align="right" bgcolor="#F7F8F9"><span class="criterio"><%IF id_proyecto= 139 or id_proyecto= 155 or id_proyecto= 156THEN %>Distrito:<%else%>Sede Distrital:<%end if%></span></td>
                ...
                <td><input type='text' class='e_input' id='txtDNI'  name='txtDNI' value='' maxlength='8'   onkeypress="return justNumbers(event);" onChange="validaNum(this.value,1,999999999)" 	 ></td>
              </tr>
              
    <tr>
    	<td  align="right" bgcolor="#F7F8F9"id="nomreg2"><span class="criterio">Cargo:</span></TD>
        <TD  class='etiqueta'><strong><%generaCargo()%></strong><span class="alerta5"></TD>
          <TD  class='etiqueta'>NRO REG.
</TD>
<td><input type='text' class='e_input' id='txtNROREG'  name='txtNROREG' value='' maxlength='6'   onkeypress="return justNumbers(event);" onChange="validaNum(this.value,1,999999999)" 	 > </td>
    </tr>
    <tr>
       ...
       <!-- <TD class='etiqueta'><span class="e_td_string">-->
          ...
          button     : "cbFechFinConv"     // el id del botón que lanzará el calendario 
            ...
            <TD  class='etiqueta'><input type='text' class='e_input' id='txtFOLIO'  name='txtFOLIO' value='' maxlength='3'   onkeypress="return justNumbers(event);" onChange="validaNum(this.value,1,999999999)" 	 ></TD>
    </tr>
    <tr>
    ...
    <TR>
    	<TD colspan="5" align="center"><span class="etiqueta"><font size="1px" ><strong>
    
    	</strong></font></span>
    	  <input name="button2" type="button" class="boton" id="button2" value="Consultar" onClick="MostrarConvocatoria()">
