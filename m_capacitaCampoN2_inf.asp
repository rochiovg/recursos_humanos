...
consulta1 = "EXEC dbo.sp_GetDatosCapacitacion_op_inf '" & lsCargo & "', '" & varccdd & "', '" & varccppTot & "', '" & varccdiTot & "', '" & tipocargo & "', '" & idProyecto & "' , '" & capacitacion & "'" 
...
<table width='98%' border="1" cellpadding="2" cellspacing="2" bordercolor="#DAE8F3">
  ...
  <tr style="font-size:14px">
          <td colspan="18" align="center">REGISTRO DE NOTAS DE CAPACITACI&Oacute;N &nbsp;OPERADOR INFORM&Aacute;TICO&nbsp;&nbsp;<img src="images/excell1.gif" onclick="ver_reporte(2)"  width="32" height="29" /></td>
          </tr>
...
<td colspan="4"><DIV style="display:none">Nota M&iacute;nima de aprobaci&oacute;n:<% if tipousuario<>"11" then %> <span class="etiqueta">
  <input  name="notMin"onkeypress="return fs_numeros(event)" class="etiqueta"  type="text" id="notMin" value="<%=nota_min%>"  onblur="guardarMin('<%=RsP.Fields("etapa")%>', this.value)" size="8" maxlength="2" />
            ...
            <td colspan="9" >&nbsp;</td>
            <td colspan="5" align="right" ><input type="HIDDEN" class='e_texto' id='txtCapacitacion' name='txtCapacitacion' maxlength="5" value="0" style="width:50" /></td>
  </tr>
	   ...
	   varvisualiza="disabled=""disabled"""
          ...
          <td width="80" class='e_td_string1'><span class="e_td_string1">
            ...
            <td align="center" class='e_td_string1'><span class="etiqueta">
              <input  <%=varvisualiza%> name="txtp1<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros(event)" class="etiqueta"  type="text" id="txtp1<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p1")%>"  onblur="guardarCAPAED('p1','<%=RsP.Fields("id_per")%>','<%=RsP.Fields("id_capa")%>',this.value, '1')" size="5" maxlength="2" />
            </span></td>
            ...
            <td width="207" class='e_td_string1'><span class="etiqueta">
              <input  <%=varvisualiza%> name="txtp3<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros(event)" class="etiqueta"  type="text" id="txtp3<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p3")%>"  onblur="guardarCAPAED('p3','<%=RsP.Fields("id_per")%>','<%=RsP.Fields("id_capa")%>',this.value, '3')" size="5" maxlength="2" />
            </span></td>
            <td width="207" class='e_td_string1'><span class="etiqueta">
              <input  <%=varvisualiza%> name="txtp4<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros(event)" class="etiqueta"  type="text" id="txtp4<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p4")%>"  onblur="guardarCAPAED('p4','<%=RsP.Fields("id_per")%>','<%=RsP.Fields("id_capa")%>',this.value, '4')" size="5" maxlength="2" />
            </span></td>
            <td width="207" class='e_td_string1'><span class="etiqueta">
              <input  <%=varvisualiza%> name="txtp5<%=RsP.Fields("id_per")%>"onKeyPress="return fs_numeros(event)" class="etiqueta"  type="text" id="txtp5<%=RsP.Fields("id_per")%>" value="<%=RsP.Fields("p5")%>"  onblur="guardarCAPAED('p5','<%=RsP.Fields("id_per")%>','<%=RsP.Fields("id_capa")%>',this.value, '5')" size="5" maxlength="2" />
            </span></td>
