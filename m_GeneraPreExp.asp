...
<% end if %>                  
                <td class='e_td_string1'>&nbsp;<span class="etiqueta">
                  <%=RsP.Fields("NotaPreCap")%>
                </span></td>
                <td class='e_td_string1'>&nbsp;<div id="tr<%=RsP.Fields("ID_PER")%>" ><%=RsP.Fields("estado_pre")%></div></td>
				
				<%  estado_n=""
				if (RsP.Fields("preseleccionado")="1") then 
					estado_n="Aprobado" 
				elseif (RsP.Fields("preseleccionado")="2") then 
					estado_n="Desaprobado" 
				elseif (RsP.Fields("preseleccionado")="3") then 
					estado_n="Aprobado No Seleccionado" 
				elseif (RsP.Fields("preseleccionado")="4") then 
					estado_n="No rindi&oacute; la prueba" 
				elseif (RsP.Fields("preseleccionado")="5") then 
					estado_n="Desaprobado - Observado por MINEDU" 
				end if  %>
