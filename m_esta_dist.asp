...
<%
  ...
  prov = Request.Form("prov")
	...
	consulta1 = "EXEC dbo.PA_COBERTURA_DIST '"&id_cc&"', '"&ccdd&"', '"&ccpp&"' "  
  ...
  <table height="20" width="20" id="Exportar_a_Excel" style="font-family:Arial; font-size:9px" border="0" bordercolor="#DFDFDF">
    <tr height="20px">
      <td align="center" valign="middle">
        <form action="reporte.asp" method="post" target="_blank" id="FormularioExportacion_">
          <img src="images/excell1.gif" style="cursor:pointer" title="EXPORTAR A EXCEL" onclick="javascript:exporta_esta_dis('<%=ccdd%>','<%=ccpp%>','<%=dpto+"*"+prov%>')"/>
   ...
   <tr height="20px">
      <th colspan="23"  bgcolor="#F2F3F4"><span class="aplicacion" style="height:30px">COBERTURA: DEPARTAMENTO <%=dpto%> - PROVINCIA <%=prov%></span></th>
   </tr>
   <tr height="20px">
      <th colspan="23"  bgcolor="#F2F3F4"><span class="aplicacion"><a href="#" onclick="verProvincias ('<%=id_cc%>', '<%=ccdd%>', '<%=dpto%>')" title="Ver Reporte a nivel provincial">Retroceder</a></span></th>
   </tr>
   <tr>
      <td width="30"  align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>Nro</strong></td>
      <td width="100" align="center" rowspan="2" bgcolor="#E4E4E4" style="font-size:9px;"><strong>DISTRITO</strong></td>
   ...
   <tr <%=col%> height="16px">
      ...
      <% else %>
      <td><a href="#" onclick="verZonas('<%=id_cc%>', '<%=RsP.Fields("ccdd")%>', '<%=RsP.Fields("ccpp")%>', '<%=RsP.Fields("ccdi")%>','<%=dpto%>' ,'<%=prov%>' , '<%=RsP.Fields("dist")%>')" title="Ver Zonas">
				<%=RsP.Fields("dist")%>
        </a>
	  </td>
      <% end if %>
